...
{{- if hasKey . "requestGPU" -}}
{{- $_ := set $merged "requestGPU" .requestGPU -}}
{{- end -}}
{{- if eq (.gpuAllocation | default "device-plugin") "dra" -}}
{{- $_ := set $merged "deviceClass" (get $head "deviceClass" | default (.deviceClass | default "gpu.nvidia.com")) -}}
{{- $_ := set $merged "gpuCount" (get $head "gpuCount" | default (.gpuCount | default 1)) -}}
{{- end -}}
{{- toYaml $merged -}}
{{- end -}}
...
claims:
  - name: gpu
{{- end }}
{{- if and (not $dra) (include "chart.hasLimits" $modelSpec | fromYaml) }}
limits:
